...
func NewSchema(embeddedType EmbeddedTypeName, version Version, definitions Definitions) *Schema {
	return &Schema{EmbeddedType: embeddedType, Version: version, Definitions: definitions}
}
				...
				if dict, ok := rec.Fields[0].(*Dictionary); ok {
					...
					for dictKey, dictValue := range *dict {
						if dictKey.Equal(NewSymbol("embeddedType")) {
							if p0 := EmbeddedTypeNameFromPreserves(dictValue); p0 != nil {
								obj.EmbeddedType = p0
								continue
							}
						}
						if dictKey.Equal(NewSymbol("version")) {
							if p1 := VersionFromPreserves(dictValue); p1 != nil {
								obj.Version = *p1
								continue
							}
						}
						if dictKey.Equal(NewSymbol("definitions")) {
							if p2 := DefinitionsFromPreserves(dictValue); p2 != nil {
								obj.Definitions = *p2
								continue
							}
						}
...
func SchemaToPreserves(d Schema) Value {
	return &Record{Key: NewSymbol("schema"), Fields: []Value{&Dictionary{NewSymbol("embeddedType"): EmbeddedTypeNameToPreserves(d.EmbeddedType), NewSymbol("version"): VersionToPreserves(d.Version), NewSymbol("definitions"): DefinitionsToPreserves(d.Definitions)}}}
}
